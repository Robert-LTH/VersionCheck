...
Try {
	...
	[string]$appScriptDate = '2018-09-03'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'atom' -AllowDeferCloseApps -DeferTimes 3 -CheckDiskSpace
		...
		Execute-Process -Path 'AtomSetup-x64.exe' -Parameters '--silent'
		
		Start-Sleep -Seconds 5
		while ((Get-Process -Name 'Update','Squirrel' -ErrorAction SilentlyContinue)) { Start-Sleep -Seconds 5}
		...
		$SMFolder = "$envUserStartMenuPrograms\GitHub, Inc"
		Move-Item -Path "$SMFolder\Atom.lnk" -Destination $envUserStartMenuPrograms
		Remove-Folder $SMFolder
		Remove-File -Path "$envUserDesktop\atom.lnk"
		#$atomexepath = (Get-ChildItem -Path "$envLocalAppData\atom" -Recurse -Filter 'atom.exe') | Select-Object -First 1
		#New-Shortcut -Path "$envUserStartMenuPrograms\Atom.lnk" -TargetPath $atomexepath
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-Process -Path "$envLocalAppData\atom\Update.exe" -Parameters '--uninstall -s'
		...
		Remove-Item -Path "$envUserStartMenuPrograms\Atom.lnk"
	}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU/W4YPQmm/JZtQmK/X4glZyb/
# LyygggQAMIID/DCCAuSgAwIBAgIJAO5U79VnHqDRMA0GCSqGSIb3DQEBCwUAMIGg
...
# AQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQM24zhmkgyzGCtWvX6
# t9mxTKGTRzANBgkqhkiG9w0BAQEFAASCAQCdqnWaCbBnnGyrml3F8RZKg5ABkJoy
# sUQPl15Ae4iXNpB1uLOBQE3PodzdHprbISLWA1qLnYxxV/Qg+OAHdb3uG1pqhRpd
# bHvm2YmltO68SSwYJQu33IMyj9urTbg1y9Pev/b9S/qy3VIi5OsxKRxzi2eZ6Ehd
# TQ8o6GQlmpR6zc6TE/r0potw6b2hmsmKRHYpEnbfIeEua791W185CKjzPV8/7hUr
# WLBqd/SavnexihHx/ORiFj4m254UmzmgPmN9MB8AZtcMnGApDtdLC+7rbEGalS4f
# eS9jEbDasxTt0+IwQq7i/y+UW/co0+g0q4eqrfme5zcMEAPOXmLr+1Ls
